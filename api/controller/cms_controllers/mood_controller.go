...
type MoodController struct {
	MoodUseCase cms_domain.IMoodUseCase
}

func (u *MoodController) GetUserApp(ctx *gin.Context) {
	page := ctx.Query("page")
	if page == "" {
		page = "1"
	}
	...
	resp, err := u.MoodUseCase.Get(ctx, intPage, intLimit, search)
	if err != nil {
		logger.Error(err.Error(), nil)
...
func (u *MoodController) CreateUserApp(ctx *gin.Context) {
	var request cms_domain.MoodRequest
	...
	resp, err := u.MoodUseCase.CreateUserApp(ctx, request)
	if err != nil {
		ctx.JSON(http.StatusUnauthorized, domain.ErrorResponse{Message: err.Error()})
		return
	}
...
func (u *MoodController) UpdateUserApp(ctx *gin.Context) {
	id := ctx.Param("id")
	if id == "" {
	...
	resp, err := u.MoodUseCase.UpdateUserApp(ctx, request, intID)
	if err != nil {
		logger.Error(err.Error(), nil)
		ctx.JSON(http.StatusUnauthorized, domain.ErrorResponse{Message: err.Error()})
		return
...
func (u *MoodController) DeleteUserApp(ctx *gin.Context) {
	id := ctx.Param("id")
	if id == "" {
	...
	resp, err := u.MoodUseCase.DeleteUserApp(ctx, intID)
	if err != nil {
		logger.Error(err.Error(), nil)
		ctx.JSON(http.StatusUnauthorized, domain.ErrorResponse{Message: err.Error()})
		return
