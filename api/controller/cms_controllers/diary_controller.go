...
type DiaryController struct {
	DiaryUseCase cms_domain.IDiaryUseCase
}

func (u *DiaryController) GetUserApp(ctx *gin.Context) {
	page := ctx.Query("page")
	if page == "" {
		page = "1"
	}
	...
	resp, err := u.DiaryUseCase.Get(ctx, intPage, intLimit, search)
	if err != nil {
		logger.Error(err.Error(), nil)
...
func (u *DiaryController) CreateUserApp(ctx *gin.Context) {
	var request cms_domain.DiaryRequest
	...
	resp, err := u.DiaryUseCase.CreateUserApp(ctx, request)
	if err != nil {
		ctx.JSON(http.StatusNotFound, domain.ErrorResponse{Message: err.Error()})
		return
	}
...
func (u *DiaryController) UpdateUserApp(ctx *gin.Context) {
	id := ctx.Param("id")
	if id == "" {
	...
	resp, err := u.DiaryUseCase.UpdateUserApp(ctx, request, intID)
	if err != nil {
		logger.Error(err.Error(), nil)
		ctx.JSON(http.StatusNotFound, domain.ErrorResponse{Message: err.Error()})
		return
...
func (u *DiaryController) DeleteUserApp(ctx *gin.Context) {
	id := ctx.Param("id")
	if id == "" {
	...
	resp, err := u.DiaryUseCase.DeleteUserApp(ctx, intID)
	if err != nil {
		logger.Error(err.Error(), nil)
